...
export function OpeningScreen({ guestName, isPersonalized, isClosing, isPreparing, error, onOpen }: OpeningScreenProps) {
  ...
  useEffect(() => {
    ...
    const revealTimer = window.setTimeout(() => setPreludePhase('leaving'), 1900)
  ...
  return (
    ...
    >
      ...
      {preludePhase !== 'visible' && <StarlightField className="z-[1] text-champagne" />}
      ...
      {isPreludeActive && (
        ...
        >
          <StarlightField className="text-champagne" />
          ...
          <PalaceSilhouette className="royal-prelude__palace pointer-events-none absolute -bottom-1 left-1/2 h-44 w-[min(1050px,155vw)] -translate-x-1/2 text-champagne/35 sm:h-60" />

          <div className="royal-prelude__content relative z-10">
            <p className="eyebrow text-champagne">A royal beginning</p>
            <TiaraMark className="mx-auto mt-5 h-16 w-40 text-champagne sm:h-20 sm:w-48" />
            <h1 id="prelude-title" className="mt-2 font-display text-[clamp(4.5rem,22vw,8rem)] font-normal leading-none tracking-[-0.06em] text-linen">
            ...
            <OrnamentalDivider className="mx-auto mt-4 h-7 w-52 text-champagne/75 sm:w-64" />
            <p className="mt-4 font-editorial text-lg italic tracking-wide text-linen/80 sm:text-xl">
              One promise. One beautiful forever.
            </p>
            <button
              ref={skipButtonRef}
              type="button"
              className="royal-prelude__skip mt-8 min-h-11 px-5 text-[9px] uppercase tracking-[0.22em] text-linen/70 transition hover:text-champagne focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-champagne"
              onClick={skipPrelude}
